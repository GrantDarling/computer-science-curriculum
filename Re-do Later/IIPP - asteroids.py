...
explosion_1_info = ImageInfo([64, 64], [128, 128], 1, 24, True)
explosion_2_info = ImageInfo([50, 50], [100, 100], 1, 81, True)
...
def process_sprite_group(set, canvas):
    for item in list(set):
        item.draw(canvas)
        item.update()
        item.collide(my_ship)
        if item.update():
            print item
            set.remove(item)
'''
def group_collide(group, other_object, comb_rad):
    global lives
    for item in list(group):
        item_pos = [(item.get_pos()[0] + item.get_radius()), (item.get_pos()[1] + item.get_radius())]
        if dist(item_pos, other_object) < comb_rad:
...
def group_group_collide(group, other_group):
    ...
    for item in list(group):
        item_pos = [(item.get_pos()[0] + item.get_radius()), (item.get_pos()[1] + item.get_radius())]
        if group_collide(other_group, item_pos, 50):
            group.remove(item)
            return True

            #if dist(item_pos, other_group) < 50:
                #print "hit"
'''
...
def group_collide(group_set, other_object):
    ...
    for object in temp_set:
        if object.collide(other_object):
            if other_object.get_radius() > 3:
                 explosion_1_group.add(Sprite(object.get_pos(), object.get_vel(), 0, 0, \
                                             explosion_2_image, \
                                             explosion_2_info, explosion_sound))

            else:
                explosion_1_group.add(Sprite(object.get_pos(), object.get_vel(), 0, 0, \
                                             explosion_1_images[1], \
                                             explosion_1_info, explosion_sound))
            if lives <= 1 and other_object.get_radius() > 30:
                  explosion_1_group.add(Sprite(other_object.get_pos(), other_object.get_vel(), 0, 0, \
...
class Sprite:
    ...
    def collide(self, other_sprite):
        # calculate and check the distance between objects
        sprite = [(self.get_pos()[0] + self.get_radius()), (self.get_pos()[1] + self.get_radius())]
        #other_object = [(other_sprite.get_pos()[0] + other_sprite.get_radius()), (other_sprite.get_pos()[1] + other_sprite.get_radius())]
        distance = dist(sprite, other_object)
        comb_rad = self.get_radius() + other_sprite.get_radius()
        #take the radius from the ship individually and
        if distance < comb_rad:
            print "nice."

            group_collide(rock_group, other_object)
    '''
      # check for collision
