...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
	  ...
	  | Line(m, b) => eval_prog(Line(m, (b + deltaY - m * deltaX)), env)
	  | VerticalLine(x) => eval_prog(VerticalLine(x + deltaX), env)
	  | LineSegment(x1, y1, x2, y2) => eval_prog(LineSegment((x1 + deltaX),(y1 + deltaY),
		   						 (x2 + deltaX),(y2 + deltaY)), env)
	  ...
	  | _ => raise Impossible "Shift can only be used with the following expressions: Point, Line, VerticalLine, LineSegment, and Intersect. Var may be used if assigned, as well."
